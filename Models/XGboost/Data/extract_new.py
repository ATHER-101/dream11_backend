...
#         for i in range(11):
#             col_name = f"opponentplayer{i+1}"
#             if not isinstance(player_stats[player][col_name], str):
#                 player_stats[player][col_name] = "Unknown_Player"
#     #add current playing xi
#     for player in player_stats:
#         team = player_stats[player]["team"]
#         current_team = teams[1] if teams[1] == team else teams[0]
        
#         current_playing_xi = data["info"]["players"].get(current_team, [])
#         for i, current_player in enumerate(current_playing_xi[:11]):
...
#             "player7", "player8", "player9", "player10", "player11","opponentplayer1","opponentplayer2","opponentplayer3","opponentplayer4",
#             "opponentplayer5","opponentplayer6","opponentplayer7","opponentplayer8","opponentplayer9","opponentplayer10","opponentplayer11"
#         ]
...
#             row[6] = player_id  # Replace with player_id (instead of player name)
#             row.extend([batting_points, bowling_points, fielding_points, total_fantasy_points])
#             row.extend([stats.get(f"player{i+1}", "Unknown_Player") for i in range(11)]) # add current columns
#             row.extend([stats.get(f"opponentplayer{i+1}", "Unknown_Player") for i in range(11)])   # Add opponent columns
#             writer.writerow(row)
...
def process_match_data(json_file, output_folder):
    ...
    for player, stats in player_stats.items():
        ...
        with open(player_file, 'a', newline='') as f:
            ...
            if row[2]== f'{"Rajiv Gandhi International Stadium, Uppal"}':
                row[2]="Rajiv Gandhi International Stadium"
            row[6] = player_id  # Replace with player_id
            row.extend([batting_points, bowling_points, fielding_points, total_fantasy_points])
